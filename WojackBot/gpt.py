...
class GPT(commands.Cog):
    ...
    @gpt.command(
        name="evil_prompt", description="Give a prompt, and the bot will respond with evilness."
    )
    async def evil_prompt(
        self, ctx, prompt: discord.Option(str, description="Ask ChatGPT something.")  # type: ignore
    ):
        ...
        await ctx.respond(j_prompt + prompt, ephemeral=True)
        LOG.info("Prompt: %s", prompt)
